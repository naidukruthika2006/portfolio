        ...
        <div className="hero grid md:grid-cols-2 items-center pt-10 xl:gap-0 gap-6 grid-cols-1">
          <div className="animate__animated animate__fadeInUp animate__delay-3s">
            <div className="flex items-center gap-3 mb-6 bg bg-zinc-800 w-fit p-4 rounded-2xl">
              <img src="./assets/kruthika-avatar.svg" className="w-10 rounded-md" />
              <q>{profileData.tagline}</q>
            </div>
            <h1 className="text-5xl font-bold mb-6">
              <ShinyText text={profileData.heroGreeting} disabled={false} speed={3} className='custom-class' />
            </h1>
            <BlurText
              text={profileData.heroDescription}
              delay={150}
              animateBy="words"
              direction="top"
              className=" mb-6"
              ...
              <div className="flex-1 text-left">
                ...
                <div className="flex flex-col sm:flex-row items-center sm:justify-between text-center sm:text-left gap-y-8 sm:gap-y-0 mb-4 w-full">
                  {profileData.stats.map((stat, index) => (
                    <div key={stat.label} data-aos="fade-up" data-aos-duration="1000" data-aos-delay={index * 200} data-aos-once="true">
                      <h1 className="text-3xl md:text-4xl mb-1">
                        {stat.value.split("/")[0]}{stat.value.includes("/") ? <span className="text-violet-500">/{stat.value.split("/")[1]}</span> : <span className="text-violet-500">{stat.value.includes("+") ? "+" : ""}</span>}
                      </h1>
                      <p>{stat.label}</p>
                    </div>
                  ))}
